...
/* FUN_0012d880 (0x12d880) — readable C lift (restored pre-naked). */
...
/* FUN_0012da90 (0x12da90) — readable C lift: send disconnect reason (ebx=ep, ax=reason). */
void FUN_0012da90(int endpoint, unsigned short reason)
{
  ...
  __asm__ volatile("movl %%ebx, %0" : "=r"(endpoint));
  __asm__ volatile("movw %%ax, %0" : "=r"(reason));
  local_reason = reason;
  if (!endpoint || reason >= 7) {
    display_assert((const char *)0x297bcc, (const char *)0x296bf0, 0x878, 1);
    system_exit(-1);
  }
  msg = encode_network_game_message(5, &local_reason, 2);
  if (!msg) {
    network_game_log((const char *)0x297b18);
    return;
  }
  len = (unsigned int)(*(unsigned short *)msg) >> 4;
  byte_swap_message_header((unsigned short *)msg, 1);
  sent = send_endpoint((int *)endpoint, (const char *)msg, (int)len);
  if (sent != (int)len) {
    err = FUN_00081c80(sent);
    network_game_log((const char *)0x297b88, err);
  }
}
...
/* FUN_0012dc20 (0x12dc20) — readable C lift: open server game (ebx=server). */
bool FUN_0012dc20(int server)
{
  unsigned int name_copy[5];
  unsigned int zero_pad[0xb];
  char stage_ok;

  __asm__ volatile("movl %%ebx, %0" : "=r"(server));
  if (!server) {
    display_assert((const char *)0x296c34, (const char *)0x296bf0, 0x961, 1);
    system_exit(-1);
  }
  network_game_log((const char *)0x297cd4);
  stage_ok = game_engine_get_current_stage((void *)(server + 0xac),
                                           (void *)(server + 0x2c));
  if (!stage_ok) {
    error(2, (const char *)0x297c94);
    return 0;
  }
  csmemcpy(name_copy, (void *)0x281c38, 0x14);
  csmemset(zero_pad, 0, 0x2c);
  network_game_generate_local_machine_name(name_copy);
  ustrncpy((wchar_t *)(server + 8), (wchar_t *)name_copy, 0xf);
  *(unsigned short *)(server + 0x26) = 0;
  *(unsigned int *)(server + 0x28) = 0;
  *(unsigned char *)(server + 0x115) = 2;
  *(unsigned char *)(server + 0x116) = 0x10;
  *(unsigned char *)(server + 0x117) =
      (unsigned char)((*(unsigned char *)(server + 0xc8) != 0) + 1);
...
/* FUN_0012e580 (0x12e580) — readable C lift (restored pre-naked). */
...
/* FUN_0012e750 (0x12e750) — readable C lift (restored pre-naked). */
...
/* network_game_server_start (0x12eb20) — readable C lift (restored pre-naked). */
...
/* network_server_manager_pregame_start (0x12eca0) — readable C lift (restored pre-naked). */
...
/* FUN_0012f5d0 (0x12f5d0) — readable C lift: encode+broadcast game settings. */
bool FUN_0012f5d0(void *server)
{
  char game_copy[0x434];
  void *game;
  void *msg;
  char ok;
  ...
  ok = 0;
  if (!server) {
    display_assert((const char *)0x296c34, (const char *)0x298ea0, 0x1c8, 1);
    system_exit(-1);
  }
  game = (void *)network_game_server_get_game(server);
  if (!game) {
    network_game_log((const char *)0x298f90);
    return 0;
  }
  csmemcpy(game_copy, game, 0x434);
  msg = encode_network_game_message(6, game_copy, 0x434);
  if (!msg) {
    network_game_log((const char *)0x298fe8);
    return 0;
  }
  ok = FUN_0012f430(server, msg);
  if (!ok)
    network_game_log((const char *)0x299028);
  return ok;
}

/* network_game_server_reset_to_pregame (0x12f690) — readable C lift (restored pre-naked). */
...
/* FUN_0012f8d0 (0x12f8d0) — readable C lift: ack (ecx=server, eax=decoded, stack=client_msg). */
char FUN_0012f8d0(int server, void *decoded_msg, void *client_message)
{
  unsigned int encoded_word;
  void *msg;
  unsigned short msg_words;
  char ok;
  int connection;
  unsigned int dest_storage;
  unsigned short unused_hdr;

  __asm__ volatile("movl %%ecx, %0" : "=r"(server));
  __asm__ volatile("movl %%eax, %0" : "=r"(decoded_msg));
  if (!server || !client_message || !decoded_msg) {
    display_assert((const char *)0x299104, (const char *)0x298ea0, 0x26a, 1);
    system_exit(-1);
  }
  encoded_word = *(unsigned int *)decoded_msg;
  msg = encode_network_game_message(3, &encoded_word, 4);
  if (!msg) {
    network_game_log((const char *)0x299130);
    return 0;
  }
  unused_hdr = 4;
  (void)unused_hdr;
  (void)*(unsigned short *)((char *)decoded_msg + 4);
  dest_storage = *(unsigned int *)client_message;
  msg_words = (unsigned short)((*(unsigned short *)msg) >> 4);
  connection = network_game_server_get_connection((void *)server);
  ok = network_connection_write((void *)connection, msg, msg_words,
                                (int)&dest_storage, 0);
  if (!ok)
    network_game_log((const char *)0x299160);
  return ok;
}

/* FUN_0012f990 (0x12f990) — readable C lift. */

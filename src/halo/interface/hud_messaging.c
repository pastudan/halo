...
/* scripted_hud_time_code_show (0xd4a20) — readable C lift. */
void scripted_hud_time_code_show(char show)
{
  if (show) {
    int t = game_time_get();
    *(int *)0x2f66e4 = t;
...
{
  char *base = (char *)*(void **)0x46bd18;
  char *slot = base + (int)param_1 * 0x460;
  if (slot[0x458] == 0) {
    return;
  }
  if (*(char *)(*(char **)0x46bd10 + 1) != 0) {
    return;
  }
  if (*(int *)(slot + 0x454) == 0) {
    return;
  }
  *(int *)(slot + 0x434 + (int)param_2 * 4) = param_3;
  slot[0x459] = (char)(slot[0x459] & (char)~(1 << (param_2 & 0x1f)));
}

/* hud_set_state_message_text (0xd4e90) — readable C lift. */
void hud_set_state_message_text(int16_t slot, int16_t index, uint16_t text_id, char flag)
{
  char *entry;
  uint8_t mask;
  ...
  if (entry[0x458] == 0)
    return;
  if ((*(char **)0x46bd10)[1] != 0)
    return;
  if (*(int *)(entry + 0x454) == 0)
    return;
  *(uint16_t *)(entry + 0x434 + (int)index * 4) = text_id;
  entry[0x436 + (int)index * 4] = flag;
  mask = (uint8_t)(1u << (index & 0x1f));
  entry[0x459] = (char)(entry[0x459] | (char)mask);
}
...
/* FUN_000d5350 (0xd5350) — readable C lift (restored pre-naked)

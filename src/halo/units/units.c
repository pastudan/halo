...
/* FUN_001a6d10 (0x1a6d10) — readable C lift. */
char *FUN_001a6d10(int unit_handle, char full_path, int16_t max_len, char *output)
{
  char *unit;
  const char *name;
  char *p;
  ...
  if (*(int16_t *)(unit + 0x338) == 0) {
    snprintf(output, (unsigned int)(int)max_len, (const char *)0x25ad08);
    return output;
  }
  name = (const char *)0x25ad08;
  if (*(int *)(unit + 0x33c) != -1)
    name = tag_get_name(*(int *)(unit + 0x33c));
  if (full_path) {
    p = (char *)name;
    while (p != NULL) {
      slash = crt_strchr(p, 0x5c);
      if (slash == NULL)
        break;
      p = slash + 1;
    }
    ...
    slash = strrchr(name, 0x5c);
    if (slash != NULL)
      name = slash + 1;
  }
  if (*(uint16_t *)(unit + 0x33a) == 0xffff) {
    snprintf(output, (unsigned int)(int)max_len, (const char *)0x257984, name);
    return output;
  }
  if (full_path) {
    seat_name = FUN_001a67b0(*(int16_t *)(unit + 0x33a), 0);
    snprintf(output, (unsigned int)(int)max_len, (const char *)0x259f40, seat_name, name);
    return output;
  }
  seat_name = FUN_001a67b0(*(int16_t *)(unit + 0x33a), 0);
  snprintf(output, (unsigned int)(int)max_len, (const char *)0x257984, seat_name);
  return output;
}
...
/* FUN_001a7ad0 (0x1a7ad0) — readable C lift.
 * Walk children of parent; for non-bit4 units, apply vitality overrides. */
void FUN_001a7ad0(int parent_handle, int param_2, int param_3)
{
  ...
  while (child != -1) {
    body = *(float *)&param_2;
    shield = *(float *)&param_3;
    obj = (char *)object_get_and_verify_type(child, -1);
    if ((obj[0xb6] & 4) == 0)
      FUN_001365d0(child, &body, &shield);
    child = FUN_000ce320(parent_handle, &iter_state);
  }
}
...
/* FUN_001a68d0 (0x1a68d0) — readable C lift (restored pre-naked) — unit dialogue speech slot allocation.
 *
...
/* unit_render_debug (0x1ad060) — readable C lift (restored pre-naked) — debug rendering for unit state.
 *
...
/* unit_leap_begin (0x1b1c70) — readable C lift (restored pre-naked) — start a leap animation.
 *
...
/* unit_throw_grenade_begin (0x1b2090) — readable C lift (restored pre-naked) — begin grenade throw animation.
 *
...
/* unit_scripting_enter_vehicle (0x1b32d0) — readable C lift (restored pre-naked) — scripted vehicle entry.
 *
...
/* FUN_001aaf40 (0x1aaf40) — readable C lift (restored pre-naked) — grenade throw initiation
...
/* FUN_001abd90 (0x1abd90) — readable C lift (restored pre-naked) — melee lunge collision damage
...
/* unit_update_running_blind (0x1af340) — readable C lift (restored pre-naked) — update running blind direction

...
void FUN_00058c40(int encounter_handle, int actor_handle, int seat_list)
{
  char buf[0x100];
  scenario_t *scenario;
  unsigned int actor16;

  if (*(unsigned char *)0x5aca59) {
    scenario = global_scenario_get();
    FUN_00054220((unsigned int)encounter_handle, (void *)scenario, buf, 0x100);
    actor16 = (unsigned int)actor_handle & 0xffff;
    error(
        2,
        (const char *)0x25d1ac,
        hs_runtime_get_executing_thread_name(),
  ...
  __asm__ volatile(
      ...
      "addl $8, %%esp"
      :
      : "r"(encounter_handle), "r"(seat_list), "X"(FUN_00058af0)
      : "eax", "ecx", "edx", "memory");
}

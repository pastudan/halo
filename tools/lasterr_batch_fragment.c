/* --- XAPILIB:lasterr.obj batch drafts (2026-07-26) --- */

/* 0x1d21f2 */
void FUN_001d21f2(void)
{

}

/* 0x1d2240 */
int xapi_GetLastError(void)
{
  return 0;
}

/* 0x1d2268 */
void __stdcall SetLastError(unsigned int error)
{
  (void)error;
}

/* 0x1d2296 */
void __stdcall XapiSetLastNTError(int status)
{
  (void)status;
}

/* 0x1d22ad */
void FUN_001d22ad(void)
{

}

/* 0x1d2367 */
void FUN_001d2367(void)
{

}

/* 0x1d23d9 */
void GetOverlappedResult(void)
{

}

/* 0x1d243e */
void FUN_001d243e(void)
{

}

/* 0x1d2518 */
void XGetLaunchInfo(void)
{

}

/* 0x1d259b */
void FUN_001d259b(void)
{

}

/* 0x1d25e0 */
int __stdcall XLaunchNewImageA(const char *image_path, void *launch_data)
{
  (void)image_path;
  (void)launch_data;
  return 0;
}

/* 0x1d26f3 */
int __stdcall XSetNicknameW(void *param_1, int param_2)
{
  (void)param_1;
  (void)param_2;
  return 0;
}

/* 0x1d292e */
void FUN_001d292e(void)
{

}

/* 0x1d29eb */
int __stdcall FUN_001d29eb(int param_1, void *param_2, int param_3)
{
  (void)param_1;
  (void)param_2;
  (void)param_3;
  return 0;
}

/* 0x1d2ad3 */
void FUN_001d2ad3(void)
{

}

/* 0x1d2ae7 */
void FUN_001d2ae7(void)
{

}

/* 0x1d2b79 */
void FUN_001d2b79(void)
{

}

/* 0x1d2bbd */
void FUN_001d2bbd(void)
{

}

#!/usr/bin/env python3
"""Third-pass HS wrapper generator: looser load detection for remaining stubs."""
...
def main() -> None:
    ...
    for i, (a, f) in enumerate(addrs):
        ...
        if len(mids) != 1:
            print("skip multi", f["addr"], [hex(x) for x in mids])
            continue
        mid = mids[0]
        ...
        for x in insns:
            ...
            # also: mov cl, byte ptr [eax+N] style already covered; movzx eax, byte
        ...
        for kind, off in refs:
            ...
            if prev is None or kind == "float":
                byoff[off] = kind
        offs = sorted(byoff)
        print(f["addr"], wname, "->", mid_name, "offs", byoff, "ret0", ret_zero)
        ...
        gen.append((f["addr"], wname, body))

    out = ROOT / "artifacts/players_hs_wrappers_gen3.c"
    out.write_text("\n\n".join(g[2] for g in gen) + "\n")
    print("GEN", len(gen), "->", out)
